            ...
            <div className="sm:flex hidden">
                {session?.user ? (
                    <div className="flex gap-3 md:gap-5">
                        ...
                        <Link href="/profile">
                            <Image src={session?.user.image} alt="User Image Profile" width={37} height={37} className="rounded-full" />
                        </Link>
                    </div>
                ) : (
                    <>
            ...
            <div className="sm:hidden flex relative">
                {
                    session?.user ? (
                        <div className="flex">
                            <Image src={session?.user.image} onClick={() => setToggledDropdown((prev) => !prev)} alt="User Image Profile" width={37} height={37} className="rounded-full" />
                            {
                                toggledDropdown && (
